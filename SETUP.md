...
Kakao Developers 앱 **mywebsite** (앱 ID `1559059`, **비즈 앱**) 를 만들고 Supabase에 연결했습니다.
...
- 동의항목:
  ...
  - 카카오계정(이메일) `account_email` = **선택 동의** ("값 없으면 카카오계정 정보 입력 요청" 켬)
    → 비즈 앱 전환 후에 활성화됨 (그 전에는 권한 없음 → KOE205 발생).
...
> Supabase gotrue 는 카카오 스코프를 `account_email profile_image profile_nickname` 로 고정
> 요청하고 클라이언트의 `scopes` 는 덮어쓰지 못한다(덧붙이기만). 그래서 세 항목 모두 카카오
> 콘솔에 설정돼 있어야 하며, 특히 `account_email` 은 **비즈 앱**에서만 켤 수 있다.
> 프로필 자동 생성 트리거는 `raw_user_meta_data` 의 `name` 을 읽으므로 카카오 닉네임이
> `profiles.full_name` 에 그대로 채워진다 (DB 변경 없음).
